<?php 
    ...
    if (isset($_POST['tambah_pasien'])) {
        ...
        $sql_cek_isentitas = mysqli_query($koneksi, "SELECT * FROM pasien WHERE no_identitas = '$identitas'") or die (mysqli_error($koneksi));
        if (mysqli_num_rows($sql_cek_isentitas) > 0) {
        ...
        if (mysqli_num_rows($sql_cek_identitas) > 0) {
            ...
            mysqli_query($koneksi, "UPDATE pasien SET tgl_daftar = '$daftar', no_identitas = '$identitas', nama_pasien = '$nama', tgl_lahir = '$lahir', jk = '$jk' , alamat = '$alamat' , no_hp = '$no_hp', status = '$status' WHERE id_pasien = '$id'") or die (mysqli_error($koneksi));
            echo "<script>window.location='data_pasien.php';</script>";
        }
        ...
        for($i = 0;$i < count($sheetData);$i++)
        {
            ...
            $no_hp = $sheetData[$i]['5'];
            mysqli_query($koneksi,"insert into pasien (id_pasien,nama_pasien,jk,alamat,no_hp) values ('','$no_identitasa','$nama_pasiens','$jk','$alamat','$no_hp')");
        }
